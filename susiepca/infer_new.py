...
def _inner_loop(X, params):
    ...
    _, _, _, params = lax.fori_loop(0, z_dim, _factor_loop, init_loop_param)
...
def _factor_loop(kdx: int, loop_params: _FactorLoopResults) -> _FactorLoopResults:
    ...
    E_zpzk = jnp.delete(E_ZZ[kdx], kdx)
    E_zzk = E_ZZ[kdx, kdx]
    Wk = W[kdx, :]
    Wnk = jnp.delete(W, kdx, axis=0)
    RtZk = params.mu_z[:, kdx] @ X - Wnk.T @ E_zpzk

    _, _, _, _, params = lax.fori_loop(
        0, l_dim, _effect_loop, (E_zzk, RtZk, Wk, kdx, params)
    )

    return loop_params._replace(params=params)
